...
import { TarifaParqueaderoService } from '../../../services/parqueadero/tarifa-parqueadero.service';
...
export class EntradaComponent implements OnInit {
  ...
  private tarifaService = inject(TarifaParqueaderoService);
  ...
  onSubmitEntrada(): void {
    if (this.formularioEntrada.valid) {
      const ticketEntrada: TicketEntradaParqueaderoRequest =
      ...
      this.ticketService.crearEntrada(ticketEntrada).subscribe({
        // el backend me devuelve un objeto TicketParqueaderoResponse
  ...
  onSubmitMensualidad(): void {
    if (this.formularioMensualidad.valid) {
      const ticketMensualidad: TicketMensualidadParqueaderoRequest =
  ...
  private generarEscPos(
    ticket: TicketParqueaderoResponse,
    tipoTicket: string
  ): string {
  ...
  reimprimirTicket(): void {
    if (this.formularioReimpresion.valid) {
      ...
      let tipoTicket = '';
    // Aca obtenemos un objeto TicketParqueaderoResponse
